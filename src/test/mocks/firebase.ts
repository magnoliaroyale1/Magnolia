...
export const mockCollection = vi.fn((db: any, path: string) => ({ path, _db: db }));
export const mockDoc = vi.fn((db: any, path: string, ...segments: string[]) => ({
  path: segments.length > 0 ? `${path}/${segments.join('/')}` : path,
  _db: db,
}));

export const mockQuery = vi.fn((...args: unknown[]) => ({ _type: 'query', _args: args }));
export const mockWhere = vi.fn((field: string, op: string, value: any) => ({ _type: 'where', field, op, value }));
export const mockOrderBy = vi.fn((field: string, dir?: 'asc' | 'desc') => ({ _type: 'orderBy', field, dir }));
